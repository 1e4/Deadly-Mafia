...
if($fetch->weapon == "Browning M2HB"){ $your_gun_power = "55"; ($kill_bullets*50000) * 10; }
...
elseif($fetch->weapon == "Jackhammer automatic shotgun"){ $your_gun_power = "25"; $gunpower = ($kill_bullets*5000) * 10; }
elseif($fetch->weapon == "Sig Sauer P229"){ $your_gun_power = "15"; $gunpower = ($kill_bullets*1000) * 10; } else {
echo "Error with weapon";	
}

if ($fetch->rank == "Tramp"){ $rank=10; }
elseif  ($fetch->rank == "Paper Kid"){ $rank=20; }
elseif  ($fetch->rank == "Theif"){ $rank=30; }
elseif  ($fetch->rank == "Robber"){ $rank=40; }
elseif  ($fetch->rank == "Gangster"){ $rank=50; }
elseif  ($fetch->rank == "Associate"){ $rank=60; }
elseif  ($fetch->rank == "Piciotto"){ $rank=70; }
elseif  ($fetch->rank == "Made Man"){ $rank=80; }
elseif  ($fetch->rank == "Capo"){ $rank=90; }
elseif  ($fetch->rank == "Consigliere"){ $rank=100; }
elseif  ($fetch->rank == "Underboss"){ $rank=110; }
elseif  ($fetch->rank == "Druglord"){ $rank=120; }
elseif  ($fetch->rank == "Godfather"){ $rank=130; }


if ($target->rank == "Tramp"){ $rank1=10; }
elseif  ($target->rank == "Paper Kid"){ $rank1=20; }
elseif  ($target->rank == "Theif"){ $rank1=30; }
elseif  ($target->rank == "Robber"){ $rank1=40; }
elseif  ($target->rank == "Gangster"){ $rank1=50; }
elseif  ($target->rank == "Associate"){ $rank1=60; }
elseif  ($target->rank == "Piciotto"){ $rank1=70; }
elseif  ($target->rank == "Made Man"){ $rank1=80; }
elseif  ($target->rank == "Capo"){ $rank1=90; }
elseif  ($target->rank == "Consigliere"){ $rank1=100; }
elseif  ($target->rank == "Underboss"){ $rank1=110; }
elseif  ($target->rank == "Druglord"){ $rank1=120; }
elseif  ($target->rank == "Godfather"){ $rank1=130; }
...
echo "Your Power was $power and his defence was $defense. You needed to shoot $kill_bullets bullets";
...
echo "<td><a href='?dl=$i->id'>Delete</a></td>";
    ...
    <tr> 
      <td height="126"><a name="send"> <input name="kill_button" type="submit" id="kill_button3" value="Submit">
      <input name="hunt_button" type="submit" id="hunt_button23" value="Submit"></td>
    </tr>
  </table>
  <p>&nbsp;</p>
</form>
