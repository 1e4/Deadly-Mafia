...
logincheck();
$username=$_SESSION['username']; ?><link href="includes/test.css" rel="stylesheet" type="text/css"><?
...
?><body background="http://i111.photobucket.com/albums/n152/cl_bullet/background.jpg"><div align="center"> <font color="#FFFFFF"><?
echo "You successfully set them free from jail!"; ?></font></div><?
$rand1 = rand(6,20) - 5;
...
echo "<META HTTP-EQUIV=\"Refresh\" CONTENT=\"0; URL=/gb/jail.php\">";
}
}}
}
if (strip_tags($_GET['mission']) == "do" && $fetch->mission == "3" && $fetch->location == "Bogota"){
$rand=rand(1,100);
$nums1 = mysql_num_rows(mysql_query("SELECT * FROM jail WHERE username='$username'"));
if ($nums1 == "0"){
if ($rand > 87){ ?><body background="http://i111.photobucket.com/albums/n152/cl_bullet/background.jpg"><font color="white"><?
echo "You busted him out of jail!"; ?></font><?
$new_money=$fetch->money + 5000000;
$new_bullets=$fetch->bullets+1000;
$new_rank=$fetch->rankpoints+rand(10,28);
mysql_query("UPDATE users SET mission='4', money='$new_money',bullets='$new_bullets', rankpoints='$new_rank', mission='4' WHERE username='$username'");
mysql_query("INSERT INTO `inbox` ( `id` , `to` , `from` , `message` , `date` , `read` , `saved` , `event_id` ) 
VALUES (
'', '$username', '$username', 'How did you find me!<br> The other gangs paid me off as dead! But thanks all the same heres all i go take it<br><b>Your rewards:</b> £5,000,000 <br>1000 Bullets ', '$date', '0', '0', '0'
)");
}else{
...
<table width=90% border=1 align=center cellpadding=2 cellspacing=0  class=thinline bordercolor=black>
  ...
  <tr>
<td  colspan=6  background=images/bg3.bmp><center>Jail</center></td></tr>
<tr bgcolor=#666666>
...
if ($fetch->mission == "3" && $fetch->location == "Bogota"){
echo "<tr><td><b>Dealer</b></td> <td>Don</td> <td>Organised crime</td> <td>Bliss Crew</td> <td><a href='jail.php?mission=do'>Bust!</a></td> <td>15 Years</td></tr>";
...
<table width="39%" border="1" align="center" cellpadding="2" cellspacing="0" bordercolor=black bgcolor="#666666" class=thinline>
  <tr>
