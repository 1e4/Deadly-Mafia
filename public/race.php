...
if(count($_POST) > 0) {
	if($max){
	    if ($max < 0){
			echo "max must be atleast 0!";
			//in
			exit();
		}elseif ($max > 50000000000){
			echo "max must be less then 50000000000!";
			//in
			exit();
		}elseif(ereg("[^[:digit:]]", $max)){
	    ...
	    if($nums == "1"){
	        ...
	        mysql_query("UPDATE race SET max='0' WHERE owner='$give' AND location='$fetch->location'");
	        print "Race Track given to $give";
	    }else{
	        print "That user does not exist";
	    }
	} elseif($drop == "yes"){
	    mysql_query("UPDATE race SET owner='0' AND max='0' WHERE owner='$username' AND location='$fetch->location'");
