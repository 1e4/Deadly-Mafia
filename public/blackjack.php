...
if($_POST['adeal']){
	...
	if($yid != ""){ 
		...
		elseif($owner != $username){ echo "Thats not your blackjack!"; } else {
			...
			mysql_query("DELETE FROM offers WHERE country='$country', bjmaxbet='0' AND casino='$casino'");
...
if ($owner == $username){
	if ($_POST['maxbet']){ 
		$newmax = $_POST['maxbet'];
		if ($newmax < 99999){
			echo "max must be atleast 100000!";
			//in
			exit();
		}elseif ($newmax > 50000000000){
			echo "max must be less then 50000000000!";
			//in
			exit();
	...
	if ($_POST['minbid']){
		$newmin = $_POST['minbid'];
		if ($newmin < 99999){
			echo "Min must be atleast 100000!";
			//in
			exit();
		}
	...
	if ($_POST['giveto']){
		...
		while($giver = mysql_fetch_row($checkgiveto)){
			...
			mysql_query("UPDATE bj SET bjowner='$giveto', bjmaxbet='100000' WHERE bjowner='$username' AND country='$state'");
		...
		<tr class=text>
		  <td width=130>Profit </td>
		  <td bgcolor=666666 width=148 colspan=3>£"; echo number_format($totalearnings); echo "</td>
		</tr>
		<tr class=text>
		  <td bgcolor=666666>Max Bet: </td>
		  <td bgcolor=666666 width=148 colspan=3><input name=maxbet type=text id=maxbet class=submit></td>
		</tr>
		<tr class=text>
		  <td bgcolor=666666>Current Max: </td>
		  <td bgcolor=666666 colspan=3>£"; echo number_format($bjmaxbet); echo "</td>
		</tr>
...
if ((isset($_POST['stand'])) && (isset($_SESSION['bet']))){
	...
	$hahahaha1=$bet*2;
		...
		if($newmoney <= 0){
			...
			mysql_query("UPDATE bj SET bjowner='$username', bjearnings='0', bjmaxbet='0' WHERE bjowner='$owner' AND country='$location'");
...
<form name=form3 method=post action=$self>
 <table width=55% border=1 cellspacing=0 cellpadding=0 bordercolor=black class=thinline>
    <tr>
      <td background=includes/grad.jpg colspan=5 class=header align=center>Blackjack</td>
    </tr>
    <tr>
...
<center>
  ...
  <tr>
  	<td background="includes/grad.jpg" colspan=30 align=center class=header>Table</td>
</tr>
	<tr  class=text>
...
<form method='post' action='blackjack.php'>
  <center><table cellspacing=0 cellpadding=0 align=center border=1 bordercolor=black class=thinline><tr><td background=includes/grad.jpg class=header align=center>Owned</td></tr><tr class=text align=center><td bgcolor=666666>This blackjack is owned by <a href=\"profile.php?user=$username\"><b>$owner</b></a></td></tr><tr class=text align=center><td bgcolor=666666>The max bet is currently <b>$bjmaxbet</b></td></tr></table>
  </center></form>";
?></body></html><? include_once"includes/footer.php" ?>