...
$fetch_points= mysql_fetch_object(mysql_query("SELECT * FROM points WHERE location='$fetch->location'"));

$points = mysql_fetch_object(mysql_query("SELECT * FROM points WHERE location='$fetch->location' LIMIT 1"));
if (strtolower($points->owner) == strtolower($fetch->username)){
include_once"pointsCP.php";
exit();
}else{

$last = $fetch->pointstime - time();
...
$with_amount=intval(strip_tags($_POST['with_amount']));
	...
	if ($with_amount > $fetch->points){
	echo "You dont have that much in the points.";
	}elseif ($with_amount <= $fetch->points){
	$money_plus = $fetch->money + $with_amount;
	$money_minus = $fetch->points - $with_amount;
	if ($money_minus == "0"){

mysql_query("UPDATE users SET money = '$money_plus', points='0', pointstime='0' WHERE username='$username'");
}elseif($money_minus != "0"){

	mysql_query("UPDATE users SET money = '$money_plus', points='$money_minus' WHERE username='$username'");
...
if(strip_tags($_POST['dep_amount']) && (strip_tags($_POST['Dep_button']))){
	...
	$points_add = $dep_amount;


	$ha = 3600*24;
		$timewait=time()+ $ha;

	mysql_query("UPDATE users SET money = '$user_loose', points='$points_add', pointstime='$timewait' WHERE username='$username'");

echo "Money deposited";
echo "<meta http-equiv=\"refresh\" content=\"0;URL=points2.php\">";
}}}}
...
mysql_query("UPDATE users SET money = money+$ban_owner_get WHERE username='$fetch_points->owner'");
...
<table width="69%" border="1" align="center" cellpadding="0" cellspacing="0" class=thinline rules=none>
  <tr> 
    <td class="header"><center>points</center></td>
  </tr>
 <tr bgcolor=black><td height=1 colspan=3></td></tr>
  <tr> 
    <td><div align="center">Welcome To The points! </div></td>
  </tr>
  <tr> 
    <td><table width="100%" border="0" cellspacing="3" cellpadding="0">
        ...
        <tr> 
          <td height="2" colspan="3">Welcome to the points of <?php echo "$fetch->location"; ?> 
          <?php if ($fetch_points->owner == "0"){ echo "This points is not owned."; }else{ echo "Your account manager name is: <a href='profile.php?viewuser=$fetch_points->owner'>$fetch_points->owner</a> 
            The owner is currently charging $fetch_points->send_intrest % 
            intrest when money is transfered to anouther account, this will be 
            taken off the amount you send the money."; } ?>
        ...
        <tr> 
          <td width="44%" height="19">points account:</td>
          <td colspan="2"><?php echo "£".makecomma($fetch->points).""; ?></td>
        </tr>
        <tr> 
          <td height="19">Timeleft:</td>
          <td height="19" colspan="2"><?php if ($fetch->pointstime <= time()){ echo "None"; }else{ echo "".maketime($fetch->pointstime).""; } ?></td>
        </tr>
        <tr> 
        ...
        <tr> 
          ...
          <td height="2"><input type="text" name="dep_amount" <?php if ($fetch->points > "0"){ echo "Disabled"; } ?>></td>
          <td height="2"><input name="Dep_button" type="submit" id="Dep_button" value="Deposit" <?php if ($fetch->points > "0"){ echo "Disabled"; } ?>></td>
        </tr>
        <tr> 
...
<?php if ($fetch_points->owner == "0"){ echo "<p><center>This points is not owned</center>"; }else{ echo "<p><center>This points is owned by <b><a href='profile.php?viewuser=$fetch_points->owner'>$fetch_points->owner</a></b></center>";  } ?>
