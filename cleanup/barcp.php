...
$owner=mysql_fetch_object(mysql_query("SELECT * FROM users WHERE username='$bar->owner'"));
if (strtolower($bar->owner) == strtolower($username)){
if (strip_tags($_POST['Submit'])){
if (strip_tags($_POST['drop']) == "Yes"){
mysql_query("UPDATE bar SET owner ='0' WHERE location='$fetch->location'");
echo "bar dropped<br>";
...
mysql_query("UPDATE bar SET owner='$give_to' WHERE location='$fetch->location'");
echo "bar given away";
}}
...
mysql_query("UPDATE bar SET profit='0' WHERE location='$fetch->location'");
echo "Profit reset.";
}
    ...
    <tr> 
      <td height="27" valign="top"> <table width="100%" border="0" align="center" cellpadding="0" cellspacing="3">
          <tr> 
            ...
            <td width="50%"><?php echo "£".makecomma($bar->profit).""; ?>(<a href="">Reset</a>)</td>
          </tr>
          <tr> 
            <td>Send To </td>
